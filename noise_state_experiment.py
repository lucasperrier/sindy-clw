"""noise_state_experiment.py

Noise sweep experiment: additive Gaussian *measurement noise* on state observations.

Goal
----
Reproduce paper-style plots showing how sparse system identification degrades as
sensor noise increases.

This script focuses on **state noise only**:
- We add noise to measured states X (trajectory samples).
- We keep oracle derivatives dX from the known CLW RHS.

For each noise level (eta):
1) add noise to X
2) perform the usual threshold sweep + model selection (Option 2)
3) evaluate coefficient recovery + support metrics
4) simulate identified model and compute trajectory error vs time
5) save plots + CSV summary to outputs/

Outputs
-------
- outputs/noise_state_summary.csv
- outputs/fig_noise_state_error_vs_time.png
- outputs/fig_noise_state_timeseries_overlay.png
- outputs/fig_noise_state_support_metrics.png
- outputs/fig_noise_state_coeff_error.png
...
def filter_drop_trajectories_below_S(
    ...
    *,
    ...
    """Keep only trajectories where S(t) >= S_min for all samples."""
...
def add_state_noise_eta(
    ...
    *,
    ...
    """Add per-dimension Gaussian noise: X_noisy = X + eta * sigma * N(0,1)."""
    eta = float(eta)
...
def _seed_for(*, base_seed: int, eta: float, trial: int) -> int:
    """Create a stable non-negative RNG seed from (base_seed, eta, trial).

    We avoid relying on signed integer arithmetic (which can go negative when using
    log10(eta)) and instead hash into a uint32 range suitable for NumPy.
    """
    # Deterministic across Python sessions (unlike built-in hash()).
    # Use float->string representation that's stable enough for our eta grid.
...
def plot_error_vs_time(curves: dict[float, tuple[np.ndarray, np.ndarray]], *, outpath: str) -> None:
    import matplotlib
    ...
    for eta, (t, err) in sorted(curves.items(), key=lambda kv: float(kv[0])):
        label = f"η={eta:g}"
        ax.plot(t, err, linewidth=1.6, label=label)

    ax.set_yscale("log")
    ax.set_xlabel("Time")
    ax.set_ylabel("Trajectory error ||x̂(t) - x(t)||₂")
    ax.set_title("CLW: trajectory error vs time for increasing state noise")
...
def plot_timeseries_overlay(
    *,
    ...
    for i, name in enumerate(STATE_NAMES):
        ...
        ax.grid(True, alpha=0.25)
        if i == 0:
            ax.set_title("CLW: time series overlay (true vs identified under state noise)")
...
def plot_support_metrics(summary_rows: list[dict], *, outpath: str) -> None:
    import matplotlib
    ...
    ax.set_title("CLW: term selection vs state noise")
    ax.grid(True, alpha=0.25)
...
def plot_coeff_error(summary_rows: list[dict], *, outpath: str) -> None:
    import matplotlib
    ...
    ax.set_title("CLW: coefficient recovery error vs state noise")
    ax.grid(True, alpha=0.25)
    fig.tight_layout()
    fig.savefig(outpath, dpi=200)
    plt.close(fig)
...
def main() -> None:
    ...
    # --- Sweep eta and aggregate metrics across trials
    summary: list[dict] = []
    ...
    for eta in cfg.eta_list:
        ...
        for trial in range(int(cfg.n_trials)):
            ...
            for thr in cfg.thresholds:
                m = fit_sindy_clw(X_list=X_noisy, dX_list=dX_list, dt=float(cfg.dt), threshold=float(thr), eps_inv=float(cfg.eps_inv))
                results.append({"threshold": float(thr), "model": m, "mse": derivative_mse(m, X_noisy, dX_list), "nnz": count_nnz(m)})
            ...
            # Aggregate across equations (macro-average)
        ...
        # Store a representative model for plots: refit using trial=0 RNG seed (deterministic)
        ...
        results0: list[dict] = []
        for thr in cfg.thresholds:
            m = fit_sindy_clw(X_list=X_noisy0, dX_list=dX_list, dt=float(cfg.dt), threshold=float(thr), eps_inv=float(cfg.eps_inv))
            results0.append({"threshold": float(thr), "model": m, "mse": derivative_mse(m, X_noisy0, dX_list), "nnz": count_nnz(m)})
    ...
    # --- Save summary table
    summary_csv = os.path.join(cfg.outdir, "noise_state_summary.csv")
    write_summary_csv(summary, path=summary_csv)
    print(f"Saved noise sweep summary to: {summary_csv}")

    # --- Plot support / coefficient metrics
    plot_support_metrics(summary, outpath=os.path.join(cfg.outdir, "fig_noise_state_support_metrics.png"))
    plot_coeff_error(summary, outpath=os.path.join(cfg.outdir, "fig_noise_state_coeff_error.png"))

    # --- Error vs time curves (use representative model per eta)
    ...
    plot_error_vs_time(curves, outpath=os.path.join(cfg.outdir, "fig_noise_state_error_vs_time.png"))

    # --- Timeseries overlay for two levels
    ...
    plot_timeseries_overlay(
        ...
        eta_low=eta_low,
        eta_high=eta_high,
        outpath=os.path.join(cfg.outdir, "fig_noise_state_timeseries_overlay.png"),
    )

    print("Saved noise-state figures to outputs/")
