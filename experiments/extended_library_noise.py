"""experiments.extended_library_noise

Extended (degree-2) library under state noise with oracle derivatives.

This fills the "overly rich library + noise" cell of the experimental matrix.
Uses the same η sweep as oracle/numerical experiments but with the extended
library instead of the physics-informed one.

Outputs
-------
- outputs/tables/coef_recovery_extended_noise_raw.csv
- outputs/tables/coef_recovery_extended_noise.csv
- outputs/figures/fig_extended_noise_overlay.png
"""
...
def main() -> None:
    ...
    lib = make_ext(eps_inv=float(cfg.eps_inv), degree=int(cfg.extended_degree))
    ...
    raw_path = os.path.join(cfg.out_tab_dir, "coef_recovery_extended_noise_raw.csv")
    _write_raw_csv(raw_path, raw_rows)

    tab_path = os.path.join(cfg.out_tab_dir, "coef_recovery_extended_noise.csv")
    ...
    plot_timeseries_overlay_three(
        ...
        outpath=os.path.join(cfg.out_fig_dir, "fig_extended_noise_overlay.png"),
        title="CLW: extended library + state noise (oracle derivatives)",
    )
    ...
    plot_error_vs_time(
        curves=curves,
        outpath=os.path.join(cfg.out_fig_dir, "fig_extended_noise_error_vs_time.png"),
        title="CLW: trajectory error vs time (extended library + state noise)",
    )
